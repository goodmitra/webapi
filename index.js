...
app.post(
  ...
  async (req, res) => {
   ...
   // var to =[919001480042,917014518593,917665914233];
        var string = to1;
        var to = string.split(",");
      ...
      if (fs.existsSync(path.concat(number) + ".json")) {
        if (Array.isArray(to)) {
          try {
            for (let x in to) {
              if (to[x].length < 12) {
                throw "value number invalid, must be greater than 12 "+to2[x]+"digit";
              }
            }
          ...
          res.end(
            JSON.stringify({
              status: false,
              message: "input type to is not array value"+to+"oo"+length,
            })
          );
        }
      } else {
